...
- The renderer reconciles DOM from `heapOre` keyed by solver body id and paints
  pane-ready `carriedOre` positions for lifted Loads. Settled pile elements use presenter
  positions directly; bag-bound falls for a one-Dwarf Crew keep the Bag arc via `fallingOre`.
...
- Pickup midpoint lift matches `pileTargetCount` and pane-ready `carriedOre` on the snapshot.
